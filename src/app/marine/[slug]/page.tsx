...
export async function generateStaticParams() {
  return MARINE_SERVICES.map((s) => ({ slug: s.slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params
  const service = MARINE_SERVICES.find((s) => s.slug === slug)
  if (!service) return {}
  return { title: `${service.title} | 3M OURCAR`, description: service.description }
}

export default async function MarineServicePage({ params }: Props) {
  const { slug } = await params
  const service = MARINE_SERVICES.find((s) => s.slug === slug)
  if (!service) notFound()
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto w-full relative z-10">
          <Link href="/marine" className="inline-flex items-center gap-2 text-xs tracking-widest uppercase mb-6 transition-colors" style={{ color: 'var(--text-muted)' }}>
            ← Back to Marine
          </Link>
          <p className="text-xs font-semibold tracking-[3px] uppercase mb-4" style={{ color: 'var(--red)' }}>Marine Tinting</p>
      ...
      <section className="py-20 px-6" style={{ background: 'var(--bg-page)' }}>
        <div className="max-w-7xl mx-auto space-y-10">
          ...
          <div className="border flex items-center justify-center" style={{ aspectRatio: '16/9', background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
            <span className="text-xs tracking-widest uppercase" style={{ color: 'var(--text-muted)' }}>[ Vessel / Installation Photo ]</span>
          </div>
